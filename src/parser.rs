...
impl Parser {
    ...
    pub fn parse(content: &str) -> Result<Eulumdat> {
        ...
        let next_line = |i: &mut usize| -> Result<&str> {
            if *i >= lines.len() {
                return Err(Error::parse(*i, "unexpected end of file"));
            }
            let line = lines[*i];
            *i += 1;
            Ok(line)
        ...
        for c in 0..mc {
            ...
            for g in 0..ldt.num_g_planes {
                let i_line = idx;
                let intensity = Self::parse_float(next_line(&mut idx)?, i_line)
                    .map_err(|_| Error::parse(i_line, format!("error reading intensity at C[{}] G[{}]", c, g)))?;
                row.push(intensity);
            }
    ...
    fn parse_int(s: &str, line: usize) -> Result<i32> {
        Self::parse_number(s)
            .map(|f| f as i32)
            .map_err(|_| Error::parse(line, format!("expected integer, got '{}'", s)))
    }

    /// Parse a float from string.
    fn parse_float(s: &str, line: usize) -> Result<f64> {
        Self::parse_number(s)
            .map_err(|_| Error::parse(line, format!("expected number, got '{}'", s)))
    }
