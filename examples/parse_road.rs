...
fn main() {
    ...
    println!("c_start index for symmetry {}: {}", ldt.symmetry as i32, c_start);
    println!();

    // Show C-angles that correspond to intensity data
    println!("C-angles for intensity data (starting at index {}):", c_start);
    ...
    println!();
    println!("First 5 G-angles: {:?}", &ldt.g_angles[..5.min(ldt.g_angles.len())]);
    println!();
