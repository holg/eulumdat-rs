...
fn render_opal_cover_scene() {
    ...
    let non_zero = image.pixels.iter().filter(|p| p[0] > 0.001 || p[1] > 0.001 || p[2] > 0.001).count();
    eprintln!("Non-zero pixels: {} / {} ({:.1}%)", non_zero, image.pixels.len(),
    ...
    eprintln!("Image rendered: {}x{}, {} bytes", image.width, image.height, bytes.len());

    // Write PPM file for visual inspection
    let mut ppm = format!("P6\n{} {}\n255\n", image.width, image.height);
    let ppm_bytes: Vec<u8> = bytes.chunks(4).flat_map(|rgba| [rgba[0], rgba[1], rgba[2]]).collect();
