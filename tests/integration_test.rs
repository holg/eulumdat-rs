...
use eulumdat::{Eulumdat, Symmetry, TypeIndicator, PhotometricCalculations, IesExporter};
...
fn test_roundtrip() {
    ...
    for (row1, row2) in ldt.intensities.iter().zip(ldt2.intensities.iter()) {
        for (v1, v2) in row1.iter().zip(row2.iter()) {
            assert!((v1 - v2).abs() < 0.01, "Intensity mismatch: {} vs {}", v1, v2);
        }
    }
}
...
fn test_ies_export() {
    ...
    // Check that intensity data is present
    assert!(ies.contains("386.8")); // First intensity value
}
